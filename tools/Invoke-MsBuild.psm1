...
#>
	...
	PROCESS
	{
		...
		$buildErrorLogFilePath = (Join-Path $BuildLogDirectoryPath $solutionFileName) + ".msbuild.err.log"
		$buildWarningLogFilePath = (Join-Path $BuildLogDirectoryPath $solutionFileName) + ".msbuild.wrn.log"
		...
		try
		{
			...
			$buildArguments = """$Path"" $MsBuildParameters /flp1:LogFile=""$buildLogFilePath"";Encoding=UTF-8;Verbosity=normal /flp2:ErrorsOnly;LogFile=""$buildErrorLogFilePath"";Encoding=UTF-8;Verbosity=normal /flp3:WarningsOnly;LogFile=""$buildWarningLogFilePath"";Encoding=UTF-8;Verbosity=normal /verbosity:q"
		...
		if ($buildSucceeded)
		{
			Write-Host ([System.IO.File]::ReadAllText($buildWarningLogFilePath)) -foregroundcolor "yellow" -backgroundcolor "black"
			# If we shouldn't keep the log around, delete it.
			if (!$KeepBuildLogOnSuccessfulBuilds)
			{
				Remove-Item -Path $buildLogFilePath -Force
				Remove-Item -Path $buildErrorLogFilePath -Force
				Remove-Item -Path $buildWarningLogFilePath -Force
			}
		}
		...
		else
		{
			...
			Write-Host ([System.IO.File]::ReadAllText($buildErrorLogFilePath)) -foregroundcolor "red" -backgroundcolor "black"
			Write-Host ([System.IO.File]::ReadAllText($buildWarningLogFilePath)) -foregroundcolor "yellow" -backgroundcolor "black"
